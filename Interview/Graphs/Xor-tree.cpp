...
void dfs(ll val, ll parent, map<ll, vector<ll>> &mp, vector<ll> &base, vector<ll> &goal, ll depth, ll evenFlip, ll oddFlip) {
    ll currentFlip = (depth&1 ? oddFlip : evenFlip);
    if(base[val-1] ^ currentFlip != goal[val-1]) {
        ans.push_back(val);
        ctr++;
        if(depth&1) {
            oddFlip ^=1;
    ...
    for(int el : mp[val]) {
        if(el != parent)
            dfs(el, val, mp, base, goal, depth+1, evenFlip, oddFlip);
    }
}
