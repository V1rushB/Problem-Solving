...
void dfs(ll val, map<ll, vector<ll>> &mp, vector<bool> &visited, ll parent) {
    if(visited[val])
        return;
    if(parent != -1) {
        ans.insert(make_pair(val,parent));
    }
    visited[val] = true;
    for(ll el : mp[val]) {
        dfs(el, mp, visited, val);
    }
}
...
void solution() {
    ...
    for(int o  = 1; o < a;o++) {
        ...
        mp[x].push_back(o+val);
        mp[o+val].push_back(x);
    }
    vector<bool> visited(a+1, false);
    dfs(k, mp, visited, -1);
    // for(auto vec : mp) {
    //     cout << vec.first << " : ";
    //     for(ll el : vec.second) {
    //         cout << el << ' ';
    //     }
    //     cout << endl;
    // }
