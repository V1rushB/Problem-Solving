...
void solution() {
    ...
    string str; cin >> str;
    vector<ll> dvdr;
    for(int o = 1; o <= str.size()/2;o++)
        if(!(str.size()%o))
            dvdr.push_back(o);
    dvdr.push_back(str.size());
    // fe(dvdr);
    pair<ll,bool> x,y;
    for(int o = 0; o < dvdr.size();o++) {
        ll s = 0;
        for(int i = 0; i < str.size();i++)
        {
            if(str[i] != str[i%dvdr[o]]) {
                s++;
            }
        }
        if(s <= 1) {
    ...
    for(int o = 0; o < dvdr.size();o++) {
        ...
        for(int i = str.size()-1; i >=0;i--) {
            if(str[i] != str[((str.size()) - dvdr[o]) + i%dvdr[o]]) {
                s++;
            }
        }
        if(s<=1) {
            // return print(min(x.first,dvdr[o]));
            y.first = dvdr[o];
            y.second = true;
            break;
        }
    }
    // return print(dvdr.back());
    // if(!y.second)
    //     y.first = dvdr.back();
    //     cout << x.first << ' ' << y.first << endl;
    return print(min(x.first,y.first));
}
