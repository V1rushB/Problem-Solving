...
bool included(ll time) {
	return time >= l && time <= r;
}

ll dpcal(ll o, int curtime) {
	if (o == dp.size())
		return 0;
	if (~dp[o][curtime])
		return dp[o][curtime];

	ll time = ((d[o] + curtime) % h), time2 = (((d[o] - 1) + curtime) % h);

	dp[o][curtime] = max(included(time) + dpcal(o + 1, time), included(time2) + dpcal(o + 1, time2));
