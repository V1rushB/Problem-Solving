...
pair<ll, ll> findInterval(vector<ll>& d) {
	bool ok = true;
	ll l(0);
	for (int o = 1; o < d.size(); o++)
	{ 
		while(o < d.size() && d[o - 1] > d[o])
		{
			if (ok)
				l = o;
			ok = false;
...
void solution() {
	...
	for (int o = 0; o < a; o++)
		cin >> d[o];
	pair<ll, ll> p = findInterval(d);
	reverse(d.begin() + p.first - 1, d.begin() + p.second);
	if (is_sorted(all(d)))
	{
		cout << "yes" << endl;
		return void(cout << p.first << ' ' << p.second << endl);
	}
