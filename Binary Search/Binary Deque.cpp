...
ll getSum(vector<ll>& d, ll l, ll r) {
	return d[r] - d[l - 1];
}

ll binarys(vector<ll>& d, ll l, ll target) {
	ll savel(l), r(d.size() - 1),temp(-1);
	while (l <= r)
	{
		ll mid = (r - l) / 2 + l;
		if (getSum(d, savel, mid) <= target)
		{
			temp = mid;
			l = mid + 1;
		}
...
void solution() {
	...
	for (int o = 1; o <= a; o++)
	{
		if (getSum(d, o, binarys(d, o, k)) != k)
			continue;
		mi = min(mi, a - (binarys(d, o, k) - o) - 1);
	}
	return print(mi == INF ? -1 : mi);
}
