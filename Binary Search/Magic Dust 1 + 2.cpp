...
bool isValid(const vector<ll> &d, const vector<ll> &x, ll dust, ll target) {
    for(int o = 0; o < d.size();o++) {
        ll div = x[o]/d[o], rem = x[o]%d[o]; // mid = 3
        if(div < target) {
            ll required = target - div;
            required*=d[o];
            required-=rem;
            if(required <= dust) {
                dust-=required;
...
ll binarys(const vector<ll> &d, const vector<ll> &x, ll dust) {
    ll l(0),r(2e9+10), temp(-1);
    while(l<=r) {
        ll mid = l + (r-l)/2;
            // cout << mid << endl;
        if(isValid(d, x, dust, mid)) {
            temp = mid;
            l = mid + 1 ;
        } else {
            r = mid - 1;
