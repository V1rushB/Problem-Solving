...
pair<bool, vector<ll>> Validate(const vector<ll> &d, ll k, const ll target) {
	ll l(0);
	vector<ll> ans;
	ans.push_back(d[l]);
	k--;
	for(int o = 1; o < d.size() && k > 0;o++) {
		if(d[o] - d[l] >= target) {
			l = o;
			ans.push_back(d[l]);
			k--;
...
vector<ll> binarys(const vector<ll> &d, const ll k) {
	ll l(0), r(d[d.size()-1] - d[0]);
	pair<bool, vector<ll>> temp,validate;
	while(l <= r) {
		ll mid = l + (r-l)/2;
		validate = Validate(d, k, mid);
		if(validate.first) {
			temp = validate;
			l = mid + 1;
		} else {
