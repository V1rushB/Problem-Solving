...
bool isValid(const vector<ll> &prefix, const ll lb, const ll time, const ll yoink) {
    return prefix[yoink] - (!lb ? 0 : prefix[lb-1]) <= time;
}

ll binarys(const vector<ll> &prefix, const ll time, ll lb) {
    ll ub(prefix.size()-1),temp(-1),l(lb);
    while(lb<=ub) {
        ll mid = lb + (ub-lb)/2;
        if(isValid(prefix, l, time, mid)) {
            temp = mid;
            lb = mid + 1;
        } else {
            ub = mid - 1;
...
void solution() {
    ...
    for(int o = 0; o < a;o++) {
        cin >> prefix[o];
        prefix[o] += (!o? 0 : prefix[o-1]);
    }
    for(int o = 0; o < a;o++) {
       ma = max(ma,binarys(prefix,x,o));
    }
    return print(max(ma,0LL));
}
