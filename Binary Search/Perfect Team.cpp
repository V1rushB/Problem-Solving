...
ll check(ll c, ll yassir, ll n,ll mid) {
	return c + yassir + n - 2 * mid;
}

ll binarys(ll c,ll m,ll n) {
	ll l(0), r(min({ c,m })),temp(-1);
	while (l <= r)
	{
		ll mid = (r - l) / 2 + l;
		if (check(c, m, n, mid) >= mid)
		{
			temp = mid;
			l = mid + 1;
		}
