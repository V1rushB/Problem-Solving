...
bool check(vector<ll>& d, ll k, ll val) {
	for (int o = 2; o < prefix.size(); o++)
	{
		/*debug("--------------");
		debug(d[o] / ((double) prefix[o - 1] + val));
		debug((k / 100.0));
		debug(val);
		debug((d[o] / ((double)(prefix[o - 1] + val)) > (k / 100.0)));
		debug("--------------");*/
		if (d[o] / ((double)(prefix[o - 1] + val)) > (k / 100.0))
			return true;
	}
	return false;
}
